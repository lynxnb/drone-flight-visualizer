...
namespace dfv {
    void MapManager::startLoad(FlightData &flightData) {
        ...
        box.llLat = -0.015 + 46.1949826657642;
        box.llLon = -0.015 + 9.73403023222216;
        box.urLat = +0.015 + 46.1949826657642;
        box.urLon = +0.015 + 9.73403023222216;


        mapMeshFuture = std::async(std::launch::async, [box, initialPos, pathNodes = std::move(pathNodes)]() mutable {
            constexpr double sparsity = 50;
            constexpr double box_size = 0.01; // Example box size
